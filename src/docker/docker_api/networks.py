...
async def list_networks(docker_session: Docker, filters: Mapping = None) -> List[Mapping]:
    params = {"filters": clean_filters(filters)}
    networks = await docker_session._query_json("networks", "GET", params=params)
    return networks


async def inspect_network(docker_session: Docker, network_id: str,
                          verbose: bool = False, scope: str = None) -> List[Mapping]:
    params = {"verbose": verbose, "scope": scope}
...
async def connect_container_to_network(docker_session: Docker, network_id, config: Mapping) -> bool:
    await DockerNetwork(docker=docker_session, id_=network_id).connect(config=config)
    return True


async def disconnect_container_from_network(docker_session: Docker, network_id, config: Mapping) -> bool:
    await DockerNetwork(docker=docker_session, id_=network_id).disconnect(config=config)
    return True
